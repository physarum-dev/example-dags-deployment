...
CHRONON_REPO_PATH = abspath(os.path.dirname(__file__))
...
class ChrononOperator(BashOperator):
    ...
    def __init__(self, conf_path, mode, conf_type, extra_args={}, *args, **kwargs):
        self.mode = mode
        ...
        self.runpy_args = f"--version={self.CHRONON_VERSION} --mode={mode} {conf_arg} {extra_args_fmt} --spark-version=3.1.1"
        # For UI rendering purposes or if chronon-ai in airflow environment.
        bash_command = f"python3 run.py {self.runpy_args} "
        env = {
            "CHRONON_REPO_PATH": CHRONON_REPO_PATH,
